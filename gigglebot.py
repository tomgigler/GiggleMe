...
def legacy_slash_help_embed(topic=None):
...
async def slash_auto_reply_create(
    ...
    # Preserve the classic Auto Reply grammar: triggers are one token. This
    # migration changes the command interface, not matching semantics.
    if re.search(r"\s", trigger):
        await interaction.response.send_message(
            "Auto Reply triggers cannot contain spaces or line breaks. "
            "Use `wildcard` when the trigger should match within a larger message."
        )
        return
...
async def slash_legacy(
    ...
    # The legacy interpreter has its own compact help mini-language.
